...
with open('response_flip.txt', 'w')as file:
    for item in response_list:
        try:
            print("URL Exists")
            if item["stores"][1]["storeLink"].find("&affid=meghana&affExtParam1=revmeup") > -1:
                print('Url is Fine')
            elif item["stores"][1]["storeLink"].find("affid=meghana&affExtParam1=revmeup") > -1:
                print('Fixing URL')
                old_url = item["stores"][1]["storeLink"]
                new_url = old_url.replace("affid=meghana&affExtParam1=revmeup","&affid=meghana&affExtParam1=revmeup")
                item["stores"][1]["storeLink"] = new_url
                ...
                print('Fixing URL')
                old_url = item["stores"][1]["storeLink"]
                new_url = old_url+"&affid=meghana&affExtParam1=revmeup"
                item["stores"][1]["storeLink"] = new_url
                res = requests.patch(BASE_URL + '/product/' + item['product_id'], data=json.dumps(item), headers=header)
                file.write("Item Fixed : " + str(item["product_id"]) + " : response code : " + str(res.status_code))
                file.write('\n')
            ...
            print(item)
            item["stores"][1]["storeLink"] = "www.flipkart.com?affid=meghana&affExtParam1=revmeup"
            res = requests.patch(BASE_URL + '/product/' + item['product_id'], data=json.dumps(item), headers=header)
