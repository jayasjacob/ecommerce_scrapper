...
class addAlert:
    ...
    # Constructor, to recieve the argument The product ID of the product
    def __init__(self, prod_info, old_price, position):
        self.prod_info = prod_info
        self.old_price = old_price
        self.position = position
        self.position = position
        # Call for identifing the users
        self.push_data()
    ...
    def push_data(self):
        ...
        for i in range(int(user_count)):
            ...
            # Create Messgae payload for each user
            store_name = self.prod_info["stores"][self.position]["storeName"]
            payload = {'message': 'The price of the product ' + self.prod_info["product_name"] + " you were stalking has been reduced from " + self.old_price.__str__()  + " to " + self.prod_info["stores"][self.position]["storePrice"].__str__(),
                       'user_id': user,
            ...
            print('Notification added for User : ', user)
